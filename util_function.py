...
def check_player_utility(player_turn, game):
    ...
    if player_turn == 0:
        for i in game.mobs_coords:
            utility -= ((game.player_x - i[1]) **
                        2 + (game.player_y - i[0])**2)**(1/2)
    if player_turn == 1:
        if key_coord:
            utility += ((game.player_x - key_coord[1]) **
                        2 + (game.player_y - key_coord[0])**2)**(1/2)
        else:
            utility += ((game.player_x - exit_coord[1]) **
                        2 + (game.player_y - exit_coord[0])**2)**(1/2)
        for i in treas_coord:
            value = ((game.player_x - i[1]) **
                     2 + (game.player_y - i[0])**2)**(1/2)
            if value <= 2:
                utility += value
            else:
